...
import numpy as np
def layout_generate(general_para):  #生成通信布局
    ...
    for i in range(N):
        ...
        for j in range(num_rx): 
            ...
            while(not got_valid_rx):
                ...
                pair_angles = np.random.uniform(low=0, high=np.pi*2)  #角度
...
def distance_generate(general_para,layout_tx,layout_rx):  ##计算每个发射器和生成器的距离，返回矩阵
...
def CSI_generate(general_para, distances):  #生成信道状态
