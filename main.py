...
def test():
    ...
    for data in test_loader:
        ...
        with torch.no_grad():
            ...
            time_gnn.append(round(math.fabs(end-start)))
...
if __name__ == '__main__':
    ...
    #使用图神经网络的方法
    train_data_list = proc_data(train_csis, train_dists, norm_train_real, norm_train_imag, train_K)  #构建的图的形式的通信图
    ...
    #训练
    train_loader = loader.DataLoader(train_data_list, batch_size=64, shuffle=True,num_workers=1)
    test_loader = loader.DataLoader(test_data_list, batch_size=test_layouts, shuffle=False, num_workers=1)
    for epoch in range(1, 20):
        loss1 = train()
    
        # loss2 = test()
        print('Epoch {:03d}, Train Loss: {:.4f}'.format(
            epoch, math.fabs(loss1)))
        scheduler.step()
    ...
    gen_tests = [40, 80, 160,300]  #链路数量和接受者数量
    for test_K in gen_tests:
        ...
        sr = wmmse.IC_sum_rate( test_csis,Y,var)
        print('WMMSE rate:',sr)
        time_wmmse.append(round(end-start,2))
        ...
        print('CGCNet rate:', math.fabs(loss2))
    # 创建折线图
    plt.plot(gen_tests, time_wmmse, marker='o',color='blue', label='WMMSE')  # 绘制第一条线，设置颜色为蓝色
    plt.plot(gen_tests, time_gnn, marker='x',color='red', label='WCGCN-CPU')  # 绘制第二条线，设置颜色为红色
