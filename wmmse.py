...
def get_mu(Pmax,Hkk,btmp,wf):  #最大限制功率，信道矩阵，中间矩阵，加权向量
    Lmu = 0
    N = Hkk.shape[1]
    ...
    return ans  #优化后的发射信号向量


def np_WMMSE_vector(b_int, H, Pmax, var_noise):  #初始发射信号，信道矩阵，最大功率限制， 噪声方差
...
def IC_sum_rate(H,p, var_noise):  #系统信道总容量
    # H1 K*K*N
    # p1 K*N
    K = H.shape[1]
    N = H.shape[-1]
